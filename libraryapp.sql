...
CREATE DATABASE `libraryapp` IF NOT EXISTS;
USE `libraryapp`;
...
CREATE TABLE `books` (
  ...
  `book_added` timestamp NOT NULL DEFAULT current_timestamp(),
  `book_updated` timestamp NOT NULL DEFAULT current_timestamp() ON UPDATE current_timestamp()
...
CREATE TABLE `book_genres` (
  ...
  `book_genre_name` varchar(200) DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
INSERT INTO `book_genres` (`book_genre_id`, `book_genre_name`) VALUES
(4, 'biography'),
(5, 'psychology'),
(6, 'business'),
(7, 'classics'),
...
INSERT INTO `users` (`user_id`, `user_name`, `user_password`, `user_role`, `created_at`, `updated_at`) VALUES
(11, 'name', '$2b$10$CUFwY4uOlMa5Jr/Yk0ELs.ZybdxuAc2IOb0OW7Ndi4lvb67t76722', 1, '2020-06-13 13:36:04', '2020-06-13 13:36:04'),
...
,`book_genre_id` int(11)
,`book_added` timestamp
,`book_updated` timestamp
,`book_genre_name` varchar(200)
);
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_book_and_genre`  AS  select `books`.`book_id` AS `book_id`,`books`.`book_title` AS `book_title`,`books`.`book_description` AS `book_description`,`books`.`book_image` AS `book_image`,`books`.`book_author` AS `book_author`,`books`.`book_status` AS `book_status`,`books`.`book_genre_id` AS `book_genre_id`,`books`.`book_added` AS `book_added`,`books`.`book_updated` AS `book_updated`,`book_genres`.`book_genre_name` AS `book_genre_name` from (`books` join `book_genres`) where `books`.`book_genre_id` = `book_genres`.`book_genre_id` ;
...
--
ALTER TABLE `book_genres`
  MODIFY `book_genre_id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=16;
...
--
ALTER TABLE `books`
  ADD CONSTRAINT `books_ibfk_1` FOREIGN KEY (`book_genre_id`) REFERENCES `book_genres` (`book_genre_id`) ON DELETE CASCADE ON UPDATE CASCADE;
COMMIT;
